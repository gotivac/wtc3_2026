...
class PackageController extends Controller
{
    ...
    public function actionAjaxDefaultDropdown()
    {
        $ids = $_POST['ids'];
        if (!empty($ids)) {
            $packages = Package::model()->findAll(array('condition' => 'id IN (' . implode(',',$ids).')'));
            $result = '';
            foreach ($packages as $package) {
                $result .= '<option value="' . $package->id . '">' . $package->concatened . '</option>';
            }
           echo $result;
        }
        echo '';
